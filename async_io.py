...
#   !! THIS CODE HAS BEEN ADJUSTED FROM THE ORIGINAL SOURCE CODE ON THE 
#      PYMODBUS LIBRARY WEBSITE !!

# Class 'ModbusServerRequestHandler' -> Function 'callback_data' -> 
...
#WE NEED INFO FROM THE DATASTORE IN THE LOOP 
...
# --------------------------------------------------------------------------- #
# Pass the Excel Information -> User Added
# --------------------------------------------------------------------------- #
...
debugLog = '/home/cornelltgod/Documents/CornellSpooky/SpookyDebug.txt'
logging.basicConfig(filename=debugLog, filemode ='w', level=logging.DEBUG)


class ModbusServerRequestHandler(ModbusProtocol, ModbusSlaveContext, ModbusSequentialDataBlock):
    ...
    def callback_connected(self) -> None:
        ...
        try:
            ...
            logging.info("Call Sucsessful, task created")
    ...
    def callback_disconnected(self, call_exc: Exception) -> None:
        ...
        try:
            logging.info("Callback Disconnected")
    ...
    async def _recv_(self):  # pragma: no cover
        ...
        try:
            ...
            #logging.info("I recieved a message")
            dataFile = '/home/cornelltgod/Documents/CornellSpooky/DATA.txt'
    ...
    #This is user added content not found in the original PyModbus library. 
    
        #Convert the byte string to a hex string. 
        dataHexString = str(data.hex())
        
        #Do we care about this? We will only read this value if the 
        #Co-Pilot is issuing a write command. 
        
        functionHex = dataHexString[2:3+1]
        
        if functionHex == "06":
            
            #Interpret hex string values to individual variables. 
            slaveHexID = dataHexString[0:1+1]
            addressHex = dataHexString[4:7+1]
            valueHex = dataHexString[8:11+1]
            
            #Convert the hex string values to usable integers. 
            #dataIntString = data.hex() 
            slaveIntID = str(int(slaveHexID, 16))
            functionInt = str(int(functionHex, 16))
            addressInt = str(int(addressHex, 16)) 
            valueInt = str(int(valueHex, 16)) 
            
            #Spacers for beauty. 
            spacer = "  --  "
            empty = "...       "
            
            #Log all usable information for debugging. 
            logging.info(empty) #Add space for readability. 
            
            logging.info("Byte String : " + str(data))
            
            logging.info(empty) #Add space for readability. 
            
            logging.info(spacer + "Hex Data" + spacer)
            logging.info("Hex Data: " + dataHexString)
            logging.info("Slave ID: " + slaveHexID)
            logging.info("Function: " + functionHex)
            logging.info("Address: " + addressHex)
            logging.info("Value: " + valueHex)
            
            logging.info(empty) #Add space for readability. 
            
            logging.info(spacer + "Int Data" + spacer)
            logging.info("Int Data: " + "Non-used")
            logging.info("Slave ID: " + slaveIntID)
            logging.info("Function: " + functionInt)
            logging.info("Address: " + addressInt)
            logging.info("Value: " + valueInt)
            ...
            dictionary = json_object #Why is this extra step here?
            
            dictionary["lastMessage"] = dataHexString
            
            i = 0
            regAddress = [None] * (numCol - 1)
            for i in range (0, numCol - 1):
                regAddress[i] = dictionary[str(controllerRegisterNames[i])]
                logging.info(controllerRegisterNames[i] + spacer + regAddress[i])
                if int(float(regAddress[i])) == int(addressInt): 
                    logging.info("Critical Register Hit")
                    dictionary["controllerRegister" + str(i+1) + "Value"] = valueInt
                    logging.info("controllerRegister" + str(i+1) + "Value" + " is loaded with : " + valueInt)
...
class _serverList:
    ...
    async def async_stop(cls):
        ...
        if os.name == "nt":
            ...
            Log.info("You're acting in asnyc_stop -> os.name == nt")
        else:
            await asyncio.sleep(0.1)
            Log.info("You're acting in asnyc_stop -> else")
